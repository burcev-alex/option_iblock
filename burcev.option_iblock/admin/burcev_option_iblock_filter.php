...
$sTableID = "tbl_burcev_option_filter_list";
...
if(($arID = $lAdmin->GroupAction()) && $FORM_RIGHT=="W" && check_bitrix_sessid())
{
	...
	{
		$arID = Array();
		$rsData = CBurcevOptionFilter::GetList($by, $order, $arFilter, $is_filtered);
	...
	foreach($arID as $ID)
	{
		...
		case "delete":
			@set_time_limit(0);
			$DB->StartTransaction();
			if(!CBurcevOptionFilter::Delete($ID))
			{
				$DB->Rollback();
...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	$arActions = Array();
	$arActions[] = array("DEFAULT"=>"Y", "ICON"=>"edit", "TITLE"=>GetMessage("BURCEV_OPTION_IBLOCK_EDIT_ALT"), "ACTION"=>$lAdmin->ActionRedirect("burcev_option_iblock_filter_edit.php?lang=".LANGUAGE_ID."&ID=$f_ID"), "TEXT"=>GetMessage("BURCEV_OPTION_IBLOCK_EDIT"));
...
if ($FORM_RIGHT=="W")
{
	...
	$aMenu[] = array(
		...
		"LINK"=>"burcev_option_iblock_filter_edit.php?lang=".LANG,
		"ICON" => "btn_new"
	);
