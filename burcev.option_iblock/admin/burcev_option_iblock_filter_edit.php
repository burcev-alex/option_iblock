...
define("HELP_FILE","burcev_option_iblock_filter.php");
	...
	if ((strlen($_REQUEST['save'])>0 || strlen($_REQUEST['apply'])>0) && $_SERVER['REQUEST_METHOD']=="POST")
	{
		...
		else
		{
			...
			if(strlen($apply) <= 0)
			{
				...
				{
					LocalRedirect("/bitrix/admin/burcev_option_iblock_filter.php?lang=".LANGUAGE_ID);
				}
			}
			else
			{
				LocalRedirect("/bitrix/admin/burcev_option_iblock_filter_edit.php?lang=".LANGUAGE_ID."&ID=".$ID);
			}
		}
	}
...
$arParser = CBurcevOptionFilter::GetByID($ID, 'burcev.option_iblock');
...
if ($ID > 0)
{
	...
	$link = "burcev_option_iblock_filter_edit.php?lang=".LANGUAGE_ID."&ID=".$ID;
...
?>
	...
	<tr>
		...
		<td width="60%">
                    ...
                    <?
					$dbCatalogSection = CIBlockSection::GetTreeList(Array("IBLOCK_ID"=>5), array("ID", "NAME", "DEPTH_LEVEL"));
                    while($arCatalogSection = $dbCatalogSection->GetNext()){
                        echo '<option value="'.$arCatalogSection["ID"].'"';
...
$tabControl->EndTab();
	...
	$tabControl->Buttons(
		array(
			"disabled" => false,
			"back_url" => "/bitrix/admin/burcev_option_iblock_filter.php?lang=".LANGUAGE_ID,
		)
	);
